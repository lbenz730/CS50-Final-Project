...
def reboundchart():
    ...
    sql = "SELECT * FROM playbyplay WHERE event_type = 'rebound' AND player = :player AND"
    sql += " (data_set = '2013-2014 Regular Season' OR data_set = '2014-2015 Regular Season')"
    rows = db.execute(sql, player = PLAYER)
    ...
    drebY = []
    for i in range(len(rows)):
        # Shot occurs 1 play before rebound
        game_id = rows[i]["game_id"]
        play_id = rows[i]["play_id"] - 1
        
        # Get shot data from database
        shot = db.execute("SELECT * FROM playbyplay WHERE game_id = :game_id AND play_id = :play_id", game_id = game_id, play_id = play_id)
        
        # Classify rebound
        if len(shot) == 1:
            if shot[0]["original_x"] != "unknown" and shot[0]["original_y"] != "unknown":
                if rows[i]["type"] == "rebound offensive":
                    orebX.append(shot[0]["original_x"])
                    orebY.append(shot[0]["original_y"])
                # else: 
                    # drebX.append(shot[0]["original_x"])
                    # drebY.append(shot[0]["original_y"])
            
    # Plot Shots for which player had the rebound
    plt.figure(figsize=(10, 8.5))
    plt.scatter(orebX, orebY, marker = "o", color = "cyan")
    plt.scatter(drebX, drebY, marker = "o", color = "magenta")
    ...
    return render_template("reboundchart.html")